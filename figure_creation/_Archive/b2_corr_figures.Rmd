...
## Correlations for Block 2 Outcomes vs NARA

```{r p_corr_ct b2}
p_corr_ct_b2 <- ggplot(b2, aes(x=Total_Time, y=NARA)) + 
  geom_point()+
...
```{r echo=TRUE}
corr_ct_b2 <- cor.test(b2$Total_Time, b2$NARA)
corr_ct_b2
```


```{r p_corr_ctnav b2}
p_corr_ctnav_b2 <- ggplot(b2, aes(x=Navigation_Time, y=NARA)) + 
  geom_point()+
  geom_smooth(method=lm)+
  xlab("B2 Navigation Time")+
  ylim(0,100)
p_corr_ctnav_b2
```

```{r echo=TRUE}
corr_ctnav_b2 <- cor.test(b2$Navigation_Time, b2$NARA)
corr_ctnav_b2
```

```{r p_corr_ctori b2}
p_corr_ctori_b2 <- ggplot(b2, aes(x=Orientation_Time, y=NARA)) + 
  geom_point()+
  geom_smooth(method=lm)+
  xlab("B2 Orientation Time")+
  ylim(0,100)
p_corr_ctori_b2
```

```{r echo=TRUE}
corr_ctori_b2 <- cor.test(b2$Orientation_Time, b2$NARA)
corr_ctori_b2
```

```{r p_corr_dt b2}
p_corr_dt_b2 <- ggplot(b2, aes(x=Distance, y=NARA)) + 
  geom_point()+
  geom_smooth(method=lm)+
  xlab("B2 Distance")+
  ylim(0,100)
...
```{r echo=TRUE}
corr_dt_b2 <- cor.test(b2$Distance, b2$NARA)
corr_dt_b2
```

```{r p_corr_speed b2}
p_corr_speed_b2 <- ggplot(b2, aes(x=Speed, y=NARA)) + 
  geom_point()+
  geom_smooth(method=lm)+
  xlab("B2 Speed")+
  ylim(0,100)
...
```{r echo=TRUE}
corr_speed_b2 <- cor.test(b2$Speed, b2$NARA)
corr_speed_b2
```

```{r p_corr_dwell b2}
p_corr_dwell_b2 <- ggplot(b2, aes(x=Mean_Dwell, y=NARA)) + 
  geom_point()+
  geom_smooth(method=lm)+
  xlab("B2 Mean Dwell Time")+
  ylim(0,100)
p_corr_dwell_b2
```

```{r echo=TRUE}
corr_dwell_b2 <- cor.test(b2$Mean_Dwell, b2$NARA)
corr_dwell_b2
```

```{r p_corr_varx b2}
p_corr_varx_b2 <- ggplot(b2, aes(x=Var_X, y=NARA)) + 
  geom_point()+
  geom_smooth(method=lm)+
  xlab("B2 Var X")+
  ylim(0,100)
...
```{r echo=TRUE}
corr_varx_b2 <- cor.test(b2$Var_X, b2$NARA)
corr_varx_b2
```


```{r p_corr_vary b2}
p_corr_vary_b2 <- ggplot(b2, aes(x=Var_Y, y=NARA)) + 
  geom_point()+
  geom_smooth(method=lm)+
  xlab("B2 Var Y")+
  ylim(0,100)
...
```{r echo=TRUE}
corr_vary_b2 <- cor.test(b2$Var_Y, b2$NARA)
corr_vary_b2
```


```{r p_corr_varz b2}
p_corr_varz_b2 <- ggplot(b2, aes(x=Var_Z, y=NARA)) + 
  geom_point()+
  geom_smooth(method=lm)+
  xlab("B2 Var Z")+
  ylim(0,100)
...
```{r echo=TRUE}
corr_varz_b2 <- cor.test(b2$Var_Z, b2$NARA)
corr_varz_b2
```


```{r p_corr_teleport b2}
p_corr_teleport_b2 <- ggplot(b2, aes(x=Teleportations, y=NARA)) + 
  geom_point()+
  geom_smooth(method=lm)+
  xlab("B2 Teleportations")+
  ylim(0,100)
p_corr_teleport_b2
```

```{r echo=TRUE}
corr_teleport_b2 <- cor.test(b2$Teleportations, b2$NARA)
corr_teleport_b2
```
