...
```{r}
# Running linear mixed model
m_varx <- lmer(Var_X ~ Block_num*Target_Name + (1|Participant), data=nav_data)
m_varx_anova <- anova(m_varx)
m_varx_anova
```

```{r}
# Jasmine's code
emm_m_varx_b <- emmeans(m_varx,~Block_num)
# can look at just Block_num, or just Target_Name; or the interaction with |
# can run the reverse: Target_Name | Block_num for contrasts 
m_varx_block <- pairs(emm_m_varx_b)
m_varx_block <- as.data.frame(m_varx_block)
```

```{r}
# Jasmine's code
emm_m_varx_t <- emmeans(m_varx,~Target_Name)
# can look at just Block_num, or just Target_Name; or the interaction with |
# can run the reverse: Target_Name | Block_num for contrasts 
m_varx_targetonly <- pairs(emm_m_varx_t)
m_varx_targetonly <- as.data.frame(m_varx_targetonly)
```

```{r}
# Jasmine's code
emm_m_varx <- emmeans(m_varx,~Block_num|Target_Name)
# can look at just Block_num, or just Target_Name; or the interaction with |
# can run the reverse: Target_Name | Block_num for contrasts 
m_varx_blocktarget <- pairs(emm_m_varx)
m_varx_blocktarget <- as.data.frame(m_varx_blocktarget)
```
...
```{r}
# Running linear mixed model
m_vary <- lmer(Var_Y ~ Block_num*Target_Name + (1|Participant), data=nav_data)
m_vary_anova <- anova(m_vary)
m_vary_anova
```

```{r}
# Jasmine's code
emm_m_vary_b <- emmeans(m_vary,~Block_num)
# can look at just Block_num, or just Target_Name; or the interaction with |
# can run the reverse: Target_Name | Block_num for contrasts 
m_vary_block <- pairs(emm_m_vary_b)
m_vary_block <- as.data.frame(m_vary_block)
```
```{r}
# Jasmine's code
emm_m_vary_t <- emmeans(m_vary,~Target_Name)
# can look at just Block_num, or just Target_Name; or the interaction with |
# can run the reverse: Target_Name | Block_num for contrasts 
m_vary_targetonly <- pairs(emm_m_vary_t)
m_vary_targetonly <- as.data.frame(m_vary_targetonly)
```

```{r}
# Jasmine's code
emm_m_vary <- emmeans(m_vary,~Block_num|Target_Name)
# can look at just Block_num, or just Target_Name; or the interaction with |
# can run the reverse: Target_Name | Block_num for contrasts 
m_vary_blocktarget <- pairs(emm_m_vary)
m_vary_blocktarget <- as.data.frame(m_vary_blocktarget)
```
...
```{r}
# Running linear mixed model
m_varz <- lmer(Var_Z ~ Block_num*Target_Name + (1|Participant), data=nav_data)
m_varz_anova <- anova(m_varz)
m_varz_anova
```
```{r}
# Jasmine's code
emm_m_varz_b <- emmeans(m_varz,~Block_num)
# can look at just Block_num, or just Target_Name; or the interaction with |
# can run the reverse: Target_Name | Block_num for contrasts 
m_varz_block <- pairs(emm_m_varz_b)
m_varz_block <- as.data.frame(m_varz_block)
```
```{r}
# Jasmine's code
emm_m_varz_t <- emmeans(m_varz,~Target_Name)
# can look at just Block_num, or just Target_Name; or the interaction with |
# can run the reverse: Target_Name | Block_num for contrasts 
m_varz_targetonly <- pairs(emm_m_varz_t)
m_varz_targetonly <- as.data.frame(m_varz_targetonly)
```

```{r}
# Jasmine's code
emm_m_varz <- emmeans(m_varz,~Block_num|Target_Name)
# can look at just Block_num, or just Target_Name; or the interaction with |
# can run the reverse: Target_Name | Block_num for contrasts 
m_varz_blocktarget <- pairs(emm_m_varz)
m_varz_blocktarget <- as.data.frame(m_varz_blocktarget)
```
